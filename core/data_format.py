...
class DataFormat:
    ...
    def __json_format(cls):
        json_text = cls.obj.ui.json_content.toPlainText()
        if not json_text:
        ...
        try:
            ...
            cls.obj.ui.json_content.setPlainText(json.dumps(json_data, ensure_ascii=False, indent=2))
    ...
    @classmethod
    def __json_copy(cls):
        if not cls.obj.ui.json_content.toPlainText():
            cls.obj.message('内容为空，不能复制', show_status=True, _type='warning')
            return
        pyperclip.copy(cls.obj.ui.json_content.toPlainText())
    ...
    def __json_compress(cls):
        if not cls.obj.ui.json_content.toPlainText():
            ...
            return
        try:
            data = json.loads(cls.obj.ui.json_content.toPlainText())
            cls.obj.ui.json_content.setPlainText(json.dumps(data, ensure_ascii=False))
